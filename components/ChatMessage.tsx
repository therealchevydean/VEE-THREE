    ...
    <div className="mt-3 border-t border-gray-500 pt-3">
      ...
      <ul className="space-y-1.5 list-none p-0">
        {parsedSources.map((source, index) => {
          ...
          const s = source as { uri: string; title: string };
          return (
            <li key={index} className="text-sm">
              <a href={s.uri} target="_blank" rel="noopener noreferrer" className="text-cyan-300 hover:underline flex items-center gap-2">
...
const ChatMessage: React.FC<ChatMessageProps> = ({ message }) => {
  ...
  const messageClasses = `max-w-xl p-4 rounded-2xl flex flex-col relative shadow-sm ${isUser
          ...
          <div className="mt-3 border-t border-indigo-400/50 pt-3 space-y-2">
            {message.files.map((file, index) => (
              <div key={index} className="flex items-center gap-2 text-xs text-indigo-100 bg-indigo-500/50 p-2 rounded-md">
                ...
                <span className="truncate" title={file?.name || ''}>{file?.name || 'Unnamed file'}</span>
              </div>
            ))}
          </div>
        )}
